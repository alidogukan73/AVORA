...
public class DataSyncActivity extends AppCompatActivity {
    ...
    private void renderNasAccount() {
        ...
        nasSyncButton.setEnabled(activeNasSession != null && !anyNasBusy
                && !nasBackupStatusLoading && !nasPhotoStatusLoading);
    }
    ...
    private void runNasTwoWayTransfer(NasSession session,
        ...
        nasExecutor.execute(() -> {
            try {
                ...
                if (latestData != null
                        && !viewModel.validateNasBackup(latestData).valid) {
    ...
    private void finishNasTwoWayFailure(NasSession session, Throwable error) {
        ...
        renderNasAccount();
        int message = "NAS_BACKUP_INVALID".equals(code)
                ? R.string.data_sync_nas_two_way_invalid_backup
                : "NAS_TIMEOUT".equals(code) || "NAS_UNAVAILABLE".equals(code)
